...
def ok_373(IS):
    ...
    result = service_373(open('Шаблоны/373-OK_Request.xml', mode='r', encoding="utf8").read(), IS, '373_ok')
    error = 0
    if result:
...
def nofound_373(IS):
    ...
    result = service_373(open('Шаблоны/373-NOFOUND_Request.xml', mode='r', encoding="utf8").read(), IS, '373_nofound')
    if result:
...
def error_data_373(IS):
    ...
    result = service_373(open('Шаблоны/373-ERROR1_Request.xml', mode='r', encoding="utf8").read(), IS, '373_err_data')
    error = 0
    if result == None:
        # пришел пустой
        error += 1
...
def error_guid_373(IS):
    ...
    result = service_373(open('Шаблоны/373-ERROR2_Request.xml', mode='r', encoding="utf8").read(), IS, '373_err_guid')
    error = 0
    if result == None:
        # пришел пустой
        error += 1
...
def test_373(IS):
    ...
    post = {
        ...
        "errors": err,
        "address": 'http://%s:%s%sSMEV/Child256.ashx' % (IS['adr'], IS['port'], IS['url'])
        }
    return post
